    ...
    <div class="page-body">
        ...
        <div class="page page-center">
            <div class="container container-tight py-4">
                ...
                <x-form class="card card-md" action="{{ route('user.update', $user->id) }}" method="put">
                    <div class="card-body">
                        <h2 class="card-title text-center mb-4">Edit user</h2>

                        <div class="mb-3">
                            <x-label class="form-label">Username</x-label>
                            <x-input type="text" class="form-control" placeholder="Enter name" name="username" id="username" value="{{ $user->username }}"/>
                        </div>
                        <div class="mb-3">
                            <x-label class="form-label">Name</x-label>
                            <x-input type="text" class="form-control" placeholder="Enter name" name="name" id="name" value="{{ $user->name }}"/>
                        </div>
                        <div class="mb-3">
                            <x-label class="form-label">Email address</x-label>
                            <x-input type="email" class="form-control" placeholder="Enter email" name="email" id="email" value="{{ $user->email }}"/>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-icon">
                                ...
                                <option value="{{ $sect->id }}" {{ ($user->section_id == $sect->id) ? 'selected' : ''  }}>{{ $sect->name }}</option>
                                @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <x-select name="role" id="role" class="form-select">
                                ...
                                <option value="3" {{ ($user->role == '3') ? 'selected' : '' }}>Reviewer</option>
                                <option value="4" {{ ($user->role == '4') ? 'selected' : '' }}>Approver</option>
                        ...
                        <div class="form-footer">
                            <x-button type="submit" class="btn btn-primary w-100">Update account</x-button>
                        </div>
                    </div>
                </x-form>
            </div>
